...
[.lead]
Node2 with the replacement modules is now ready to boot. Upgrading by swapping the system modules involves moving only the console and management connections. This section provides the steps required to boot node1 with the replacement modules listed in the <<supported-systems,supported systems matrix>>.
