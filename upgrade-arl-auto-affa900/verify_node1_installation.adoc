...
+
`network int migrate -vserver Cluster -lif <cluster_lif1> -destination-node <node1_name> -destination-port e1a`

.	Verify that node1 and node2 are part of the same cluster and that the cluster is healthy:
+
`cluster show`
+
[NOTE]
