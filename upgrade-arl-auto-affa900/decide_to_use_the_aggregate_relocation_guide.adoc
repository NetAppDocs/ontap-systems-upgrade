...
[.lead]
There are several aggregate relocation (ARL) methods for upgrading controller hardware. This content describes how to upgrade storage controllers in an HA pair by converting the existing system to the replacement system, keeping the existing system chassis and disks. You should only use this complex procedure if you are an experienced administrator.
...
== Supported for this ARL upgrades
You can use this ARL procedure under the following circumstances:

* You controller upgrade is listed in the <<supported-systems,supported systems matrix>>.
...
* You're running the minimum ONTAP version for your upgrade. For more information, <<supported-systems,Supported system upgrade combinations>>.
...
== Not supported for this ARL upgrades
