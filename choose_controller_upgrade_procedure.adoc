...
== Step 1: Select the correct upgrade procedure
...
When possible, the new nodes must support the ONTAP version running on the original nodes. If this is not possible, link:https://docs.netapp.com/us-en/ontap/upgrade/prepare.html[upgrade the ONTAP version^] on the original nodes to the highest supported version. You cannot have a version difference greater than four between the original and new nodes. For example, ONTAP 9.8 and 9.12.1 is supported; however, ONTAP 9.8 and 9.13.1 is not supported. link:https://docs.netapp.com/us-en/ontap/upgrade/concept_mixed_version_requirements.html[Learn more about mixed version ONTAP clusters^].

| Do I want to do a disruptive upgrade?	
| link:upgrade/upgrade-decide-to-use-this-guide.html[Upgrade by moving storage].

This requires a new system with its own storage and shelves to store the data from the original system. You can convert from a controller shelf for certain upgrades.
...
+
This requires adding the new nodes, moving the volumes and LIFs to the new nodes, and removing the nodes you want to remove from the cluster. If you are upgrading a two-node switchless cluster, convert it to a switched-attached cluster using a pair of cluster switches before adding new nodes.
...
* Convert the original system to a drive shelf and move storage:

** link:upgrade/upgrade_aff_a250_to_aff_a400_ndu_upgrade_workflow.html[Convert AFF A250 to a drive shelf and move storage to AFF A400]
** link:upgrade/convert-to-drive-shelf.html[Convert AFF A150, AFF A220, or FAS2820 to a drive shelf and move storage to a supported replacement system]

| Do I have a modular system?

*Note*: In a modular system, the controller has no internal disks; all storage is in externally attached shelves.
...
| link:upgrade-arl-auto-in-chassis/index.html[Upgrade by using ARL, keeping the same chassis, disks, and the data stored on the disks].
...
Before beginning your upgrade procedure, review all of the questions listed in the table below.

[cols=2*,options="header",cols="30,70"]
|===
| Ask yourself…	| If your answer is yes, you need to...
...
| Does an FC SAN configuration exist on the cluster of my original system?

| Validate the SAN zoning configuration for all nodes on both the original and replacement system. Refer to link:https://docs.netapp.com/us-en/ontap/san-config/fibre-channel-fcoe-zoning-concept.html[Learn about FC and FCoE zoning with ONTAP systems^] and link:https://docs.netapp.com/us-en/ontap/san-config/fc-fcoe-recommended-zoning-configuration.html#dual-fabric-zoning-configurations[Recommended FC and FCoE zoning configurations for ONTAP systems^].
...
This example provides possible answers for a FAS9500 to FAS90 controller upgrade.
...
| Am I running a different software version on the original and new nodes?
| No. Both FAS9500 and FAS90 run ONTAP 9.17.1.
...
| Does an FC SAN configuration exist on the cluster of my original system?
| Yes. I have validated the SAN zoning configuration on all nodes of both the original and replacement system. 
