...
[.lead]
You must make sure that the physical ports on node2 map correctly to the physical ports on node4. This enables node4 to communicate with other nodes in the cluster and with the network after the upgrade.
...
.. As the reachability repair is performed, ONTAP attempts to place the ports in the correct broadcast domains. However, if ONTAP cannot determine a port’s reachability and the port does not correspond to any of the existing broadcast domains, ONTAP tries to create new broadcast domains for these ports. As required, you can delete the newly created broadcast domains if all their member ports will become member ports of the interface groups. Delete broadcast domains:
+
...
. [[man_map_2_Step5]]Beginning with ONTAP 9.8, ONTAP automatically modifies the home ports of LIFs if the ports are moved between broadcast domains during the network port reachability repair procedure. If a LIF’s home port was moved to another node, or is unassigned, that LIF is presented as a displaced LIF. Restore the home ports of displaced LIFs whose home ports either no longer exist or were relocated to another node.
...
.What's next?
After mapping ports from node2 to node4, link:move_nas_lifs_node2_from_node3_node4_verify_san_lifs_node4.html[move NAS data LIFs owned by node2 from node3 to node4 and verify SAN LIFs on node4].
