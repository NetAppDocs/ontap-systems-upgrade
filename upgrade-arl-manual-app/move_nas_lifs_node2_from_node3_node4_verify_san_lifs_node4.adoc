...
.What's next?
After moving NAS data LIFs owned by node2 from node3 to node4 and verifying SAN LIFs on node4, link:worksheet_information_before_moving_san_lifs_node4.html[record record the `adapter` and `switch-port` information for each LIF].
// Clean-up, 2022-03-09
