...
The following table describes the high-level tasks for each stage and the state of aggregate ownership at the end of the stage. Detailed steps are provided later in the procedure:

[cols="35,65"]
|===
| Stage | Steps

| link:determine_aggregates_on_internal_drives.html[Stage 1: Prepare for upgrade]
a| During Stage 1, if necessary, you confirm that internal disk drives do not contain root aggregates or data aggregates, prepare the nodes for the upgrade, and run prechecks. If necessary, you rekey disks for Storage Encryption and prepare to netboot the new controllers.
...
| link:relocate_non_root_aggr_node1_node2.html[Stage 2: Retire node1]
...
| link:record_node2_information.html[Stage 4: Retire node2]
...
| link:install_boot_node4.html[Stage 5: Install and boot node4]
a| During Stage 5, you install and boot node4 and map the cluster and node-management ports from node2 to node4. If you are using NetApp Volume Encryption (NVE), you restore key-manager configuration. Then, you move data LIFs and SAN LIFs belonging to node2 from node3 to node4, relocate node2 aggregates from node3 to node4, and move the data LIFs and SAN LIFs owned by node2 to node3.
...
| link:manage_authentication_kmip.html[Stage 6: Complete the upgrade]
...
NOTE: The storage virtual machine (SVM) disaster recovery updates are not interrupted as per the schedules assigned.
