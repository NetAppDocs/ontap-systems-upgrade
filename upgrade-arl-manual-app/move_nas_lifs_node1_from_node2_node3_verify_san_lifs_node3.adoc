...
.What's next?
After moving NAS data LIFs owned by node1 from node2 to node3 and verifying SAN LIFs on node3, link:worksheet_information_before_moving_san_lifs_node3.html[record record the `adapter` and `switch-port` information for each LIF].
// Clean-up, 2022-03-09
