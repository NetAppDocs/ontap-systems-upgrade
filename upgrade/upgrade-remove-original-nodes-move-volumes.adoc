...
If the partner node is selected as the new master, you need to verify if it holds epsilon. If the partner node holds epsilon, you need to move epsilon to a different node that is remaining in the cluster before before making it ineligible. You do this when you repeat these steps to remove the partner node.

Do not set eligibility to "false" on a node that holds epsilon.
====
