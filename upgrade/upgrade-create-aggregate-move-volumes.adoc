...
+
Both the new aggregate and the old aggregate from which the volume is being moved must be in the same SVM.
...
.What's next?
After creating an aggregate and moving volumes to the new nodes, link:upgrade-move-lifs-to-new-nodes.html[move non-SAN data LIFs and cluster-management LIFs to the new nodes].
