...
.Steps
. Turn on the power to node3, and then immediately enter Ctrl-C at the console terminal to access the LOADER prompt.
+
...
+
Enter username/password if applicable, or select Enter to continue.
...
.What's next?
After setting up the new nodes, optionally, link:upgrade-move-internal-drives.html[move internal drives from the original node] or link:upgrade-convert-node-to-shelf.html[convert the original node to a drive shelf]. Then, link:upgrade-attach-shelves-reassign-disks.html[attach storage shelves and reassign disk ownership].
