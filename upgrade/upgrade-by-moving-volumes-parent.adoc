...
summary: 'If you are upgrading by moving volumes, you prepare the original nodes and join the new nodes to the cluster. You move volumes to the new nodes, configure LIFs, and remove the original nodes from the cluster. Upgrading by moving volumes is a nondisruptive procedure.'
---
= Learn about upgrading by moving volumes
:icons: font
:imagesdir: ../media/
...
To upgrade by moving volumes, you prepare the original nodes and join the new nodes to the cluster. You move volumes to the new nodes, configure LIFs, and remove the original nodes from the cluster. 
...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-7.png[Seven]link:upgrade-remove-original-nodes.html[Remove the original nodes from the cluster]
[role="quick-margin-para"]
After the volumes have been moved to the new nodes, you remove the original nodes from the cluster. When you remove a node, the node's configuration is erased and all disks are initialized.
...
.What's next?
After learning about upgrading by moving volumes, link:upgrade-prepare-when-moving-volumes.html[prepare for the upgrade].
