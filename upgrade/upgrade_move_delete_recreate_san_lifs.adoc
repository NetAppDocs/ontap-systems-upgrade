...
If the cluster is in a SAN environment, you must delete any SAN LIFs that you no longer require from the original nodes before you can remove the original nodes from the cluster.
...
.What's next?
After moving, deleting, or creating SAN LIFs, link:upgrade-remove-original-nodes.html[remove the original nodes from the cluster].
