...
[[return-admin]]
[start=12]
. Repeat Steps <<disable-ha,2>> to <<return-admin,11>> to remove the failover partner from the cluster.
...
.What's next?
After removing the original nodes, link:upgrade-complete-move-volumes.html[complete the move volumes upgrade].
