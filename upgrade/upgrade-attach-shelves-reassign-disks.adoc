...
+
CAUTION: You must not connect new shelves to node3 or node4 during this upgrade procedure. You can nondisruptively connect new shelves to the system after completing the controller upgrade.
...
** The parameter `_node2_sysid_` is the value you recorded in  _Shutting down the original nodes_, link:upgrade-shutdown-remove-original-nodes.html#shutdown_node_step5[Step 5].
** Specify the parameter `_-p partner_sysid_` only when shared disks are present.
--
====
+
