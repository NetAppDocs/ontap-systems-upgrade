...
+
`boot_ontap menu`
. At the boot menu prompt, type `wipeconfig` and then select Enter.
+
...
----
+
The system initiates the `wipeconfig` procedure and reboots. When the procedure is complete, the system returns to the boot menu.

. From the boot menu, enter 8 to reboot the node, and enter Ctrl-C during AUTOBOOT to stop the node at the LOADER prompt.

.What's next?
After resetting the default configuration on the new nodes, link:upgrade-install-new-nodes.html[install the new nodes].
