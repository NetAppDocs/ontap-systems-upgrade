...
+
`boot_ontap menu`
. At the boot menu prompt, enter `9a` and then select Enter.
+
...
.What's next?
After removing ownership of disks attached to the new nodes, link:upgrade-reset-default-configuration-node3-and-node4.html[reset the default configuration on the new nodes].
