...
----
+
NOTE: Your system might not have intercluster LIFs. You have a cluster management LIF only on one node of a node pair. A cluster management LIF is displayed in the example output of link:record_node1_information.html#step1[Step 1] in _Record node1 port information_.
...
+
The broadcast domains need to be mapped to the ports on the new controller later in the procedure.
