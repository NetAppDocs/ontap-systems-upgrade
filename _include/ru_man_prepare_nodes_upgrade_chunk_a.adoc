...
+
Refer to link:other-references.html[References] to link to _HA pair management_ and follow the _Best practices for HA pairs_ section. Neither of the original nodes should be running at more than 50 percent utilization; if a node is running at less than 50 percent utilization, it can handle the loads for both nodes during the controller upgrade.
...
For information about unlocking the user accounts, refer to link:other-references.html[References] to link to the _System Administration Reference_.
====

.. Refer to link:other-references.html[References] to link to the _NetApp Support Site_ and download the Performance and Statistics Collector (Perfstat Converged).
+
...
. Refer to link:other-references.html[References] to link to the _NetApp Support Site_ and open a support case on the NetApp Support Site.
+
...
+
You must physically check node3 and node4 to see if the NVMEM or NVRAM batteries are charged. For information about the LEDs for the model of node3 and node4, refer to link:other-references.html[References] to link to the _Hardware Universe_.
+
...
+
If neither node1 nor node2 owns aggregates for which it is the current owner (but not the home owner), the system returns a message similar to the following example:
+
....
...
+
For detailed information about the `network interface revert` command, refer to link:other-references.html[References] to link to the _ONTAP 9 Command reference_.

. Check whether node1 or node2 owns any failed disks:
+
`storage disk show -nodelist _node1,node2_ -broken`
+
If any of the disks have failed, remove them, following instructions in the _Disk and aggregate management with the CLI_. (Refer to link:other-references.html[References] to link to _Disk and aggregate management with the CLI_.)
