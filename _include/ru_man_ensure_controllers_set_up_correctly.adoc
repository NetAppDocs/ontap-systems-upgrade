...
`system license show`
+
You can compare the output with the output that you captured in link:prepare_nodes_for_upgrade.html#man_prepare_nodes_step29[Step 29] of _Prepare the nodes for upgrade_.
...
+
Go to link:other-references.html[References] to link to the _System Administration Reference_ for information about the SPs and the _ONTAP 9 Command reference_ for detailed information about the `system service- processor network modify` command.
//2022 SEP 22, BURT-1481976
. If you want to set up a switchless cluster on the new nodes, go to link:other-references.html[References] to link to the _Network Support Site_ and follow the instructions in _Transitioning to a two-node switchless cluster_.
