You must install node4 in the rack, transfer node2 connections to node4, and boot node4. You must also reassign AFF A800 or AFF C800 controller, any node2 spares, any disks belonging to root, and any non-root aggregates that were not relocated to node3 earlier.
...
[IMPORTANT]
====
* If you upgrading an AFF A800 or AFF C800 controller, you must ensure that all drives in the chassis are firmly seated against the midplane before removing node2. For more information, see link:../upgrade-arl-auto-affa900/replace-node1-affa800.html[Replace the AFF A800 or AFF C800 controller modules]. 
...
* If you are upgrading a system with storage disks, you must complete this entire section and then proceed to the section link:set_fc_uta_uta2_config_node4.html[Set the FC or UTA/UTA2 configuration on node4], entering commands at the cluster prompt.
====
