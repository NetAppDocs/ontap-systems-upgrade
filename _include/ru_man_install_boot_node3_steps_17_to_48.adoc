...
. Enter CTRL-C to display the boot menu.
...
|Go to <<man_install3_step13,Step 13>>.
|Has the correct or current version of ONTAP on node3
|Go to <<man_install3_step18,Step 18>>.
|===

. [[man_install3_step13]]Configure the netboot connection by choosing one of the following actions.
+
...
. If you are prompted to continue the procedure, enter `y`, and when prompted for the package, enter the following URL:
+
...
. [[man_install3_step18]]Select *(5) Maintenance mode boot* by entering `5`, and then enter `y` when prompted to continue with the boot.
...
. [[man_install3_step21]]Reassign node1's spare disks, any disks belonging to the root, and any non-root aggregates that were not relocated to node2 earlier in link:relocate_non_root_aggr_node1_node2.html[Relocate non-root aggregates from node1 to node2].
+
...
. [[man_install3_step42]]Verify the partner system ID is set correctly as noted in <<man_install3_step21,Step 21>> under -p switch:
+
