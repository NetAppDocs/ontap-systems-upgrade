...
Some or all aggregates have been relocated from node1 to node2, and HA is still enabled. Node2 takes over node1's root volume and any non-root aggregates that were not relocated.
...
When node1 enters the `waiting for giveback state`, node2 gives back all the node1 non-root aggregates.

.Steps

. Complete link:relocate_non_root_aggr_node1_node2.html#relocate-step1[Step 1] in the section _Relocate non-root aggregates from node1 to node2_ again.
. Continue with the node-pair upgrade procedure.
...
. Continue with the node-pair upgrade procedure.
+
You might see some changes in the output of the `storage failover show` command, but that is typical and does not affect the procedure. See the troubleshooting section link:issues_multiple_stages_of_procedure.html#storage-failover-command[Unexpected storage failover show command output].
...
Node1 takes over all of node2's aggregates as well any of its own aggregates that it had relocated to node2. When node2 enters the `Waiting for Giveback` state, node1 gives back all of node2's aggregates.
...
. Bring up node2.
+
A client outage occurs for all aggregates while node2 is booting.
...
Node3 does not take over following a node2 crash as HA is already disabled.
...
Node2 has relocated some or all of its aggregates to node3, which serves data from aggregates that were relocated. HA is disabled.

.About this task

There is a client outage for aggregates that were not relocated.

.Steps

. Bring up node2.
. Relocate the remaining aggregates by completing link:relocate_non_root_aggr_node2_node3.html#verify-partner-sys-id[Step 1] through link:relocate_non_root_aggr_node2_node3.html#relocate-step5[Step 5] in the section _Relocate non-root aggregates from node2 to node3_.
. Continue with the node-pair upgrade procedure.
...
If node3 crashes while node2 is relocating aggregates to node3, node2 aborts the relocation of any remaining aggregates.
...
. Bring up node3.
. Complete link:relocate_non_root_aggr_node2_node3.html#relocate-step5[Step 5] again in the section _Relocate non-root aggregates from node2 to node3_.
...
. Bring up node3.
+
There is a client outage for all aggregates.
. Continue with the node-pair upgrade procedure.
...
. Bring up node3.
. Relocate the remaining aggregates that belonged to node2 by repeating link:relocate_node2_non_root_aggr_node3_node4.html#man_relocate_3_4_Step1[Step 1] through link:relocate_node2_non_root_aggr_node3_node4.html#step3[Step 3] in the section _Relocate node2's non-root aggregates from node3 to node4_.
...
. Bring up node4.
. Relocate the remaining aggregates that belonged to node2 by again completing link:relocate_node2_non_root_aggr_node3_node4.html#man_relocate_3_4_Step1[Step 1] through link:relocate_node2_non_root_aggr_node3_node4.html#step3[Step 3] in _Relocate node2's non-root aggregates from node3 to node4_.
. Continue with the node-pair upgrade procedure.
