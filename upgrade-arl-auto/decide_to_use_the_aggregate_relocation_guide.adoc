...
[.lead]
There are several aggregate relocation (ARL) methods for upgrading controller hardware. This procedure describes how to use ARL with "system controller replace" commands to upgrade the storage controllers in an HA pair with new controllers while keeping the existing data and disks. You should only use this complex procedure if you are an experienced administrator. 
