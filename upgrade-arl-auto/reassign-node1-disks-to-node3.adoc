...
. If the system goes into a reboot loop with the message `no disks found`, this is because it has reset the ports back to the target mode and therefore is unable to see any disks. Continue with <<auto_check3_step3,Step 3>> to <<auto_check3_step8,Step 8>> node node3 to resolve this issue.
