...
summary: "If required, restore key-manager configuration on node3 when upgrading to controllers introduced in ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
.What's next?
After restoring key-manager configuration on node3, link:move_non-root_aggr_and_nas_data_lifs_node1_from_node2_to_node3.html[move non-root aggregates and NAS data LIFs owned by node1 from node2 to node3].
