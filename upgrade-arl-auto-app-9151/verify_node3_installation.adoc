...
summary: "Verify that the node1 physical ports map correctly to node3 and restore network configuration on node3 when upgrading to controllers introduced in ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
[.lead]
You must verify that the physical ports from node1 map correctly to the physical ports on node3. This enables node3 to communicate with other nodes in the cluster and with the network after the upgrade.

.About this task

Refer to link:other-references.html[References] to link to the _Hardware Universe_ to capture information about the ports on the new nodes. You will use the information later in this section.

Physical port layout might vary, depending on the model of the nodes. When the new node boots up, ONTAP tries to determine which ports should host cluster LIFs in order to automatically come into quorum.

If the physical ports on node1 do not map directly to the physical ports on node3, the subsequent section <<restore_network_configuration_on_node3_9151,Restore network configuration on node3>> must be used to repair the network connectivity.
...
At this point in the procedure, the operation pauses as node3 joins quorum.

== Step 1: Verify that node3 is in quorum

Verify that node3 is in quorum. Then, resume the controller replacement operation
...
. Controller replacement pauses for intervention with the following message:
+
....
...
For more information on creating and re-creating VLANs, interface groups, and broadcast domains, refer to link:other-references.html[References] to link to _Network Management_.
...
`network port reachability show`
+
As the reachability repair is performed, ONTAP attempts to place the ports in the correct broadcast domains. However, if ONTAP cannot determine a port’s reachability and the port does not belong to any of the existing broadcast domains, ONTAP creates new broadcast domains for these ports.
...
.What's next?
After verifying the node3 installation, link:restore_key-manager_configuration_node3.html[restore key-manager configuration on node3].
