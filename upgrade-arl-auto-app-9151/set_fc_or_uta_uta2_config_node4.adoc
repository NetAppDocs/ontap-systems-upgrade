...
. [[auto_check_4_step12]]On node4, go to the boot menu and using 22/7, select the hidden option `boot_after_controller_replacement`. At the prompt, enter node2 to reassign the disks of node2 to node4, as per the following example.
+
...
. [[auto_check_4_step19]]At the loader prompt, boot up:
+
`boot_ontap menu`
+
