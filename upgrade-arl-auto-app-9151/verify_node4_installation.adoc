...
summary: "Verify that node2's physical ports map correctly to node4 and restore network configuration on node4 when upgrading to controllers introduced in ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
[.lead]
You must verify that the physical ports from node2 map correctly to the physical ports on node4. This enables node4 to communicate with other nodes in the cluster and with the network after the upgrade.
...
If the physical ports on node2 do not map directly to the physical ports on node4, the subsequent section <<restore_network_configuration_on_node4_9151,Restore network configuration on node4>> must be used to repair network connectivity.
...
At this point in the procedure, the operation pauses as node4 joins quorum.

== Step 1: Verify that node4 is in quorum

Verify that node4 is in quorum. Then, resume the controller replacement operation.
...
.What's next?
After verifying the node4 installation, link:restore_key-manager_config_node4.html[restore key-manager configuration on node4].
