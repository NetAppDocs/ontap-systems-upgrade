...
summary: "Prepare the nodes and run a series of prechecks when upgrading to controllers introduced in ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
[.lead]
The controller replacement process begins with a series of prechecks. You also gather information about the original nodes for use later in the procedure. If any prechecks fail, you must correct aggregate ownership before you can continue with the upgrade.

== Step 1: Prepare node1 and node2

Start the controller replacement process and run prechecks on node1 and node2. If required, determine the type of self-encrypting drives that are in use.
...
Check if the build on node3 and node4 is a NODAR (no data at rest encryption) image. If the build is a NODAR image, you need to install a DARE (data at rest encryption) image.

.About this task

Perform the following steps on node3 and node4.
...
systemshell
----
. [[check-build]]Check if the build is a NODAR image:
+
[source,cli]
----
cat /BUILD
----
+
In the command output, check if `build-type` is a NODAR image, for example, `DEBUG SIM NODAR`. 
+
NOTE: If `build-type` is not a NODAR image, the build is a DARE image.
. If the build is a NODAR image, install a DARE image.

.. Exit systemshell and the boot menu:
+
[source,cli]
----
halt
----
.. Install a DARE image. For more information, refer to the Knowledge Base article link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Netbooting_a_FAS_and_AFF_Controller_in_ONTAP_9[Netbooting a FAS and AFF Controller in ONTAP 9^].

.What's next?
After preparing the nodes, link:complete-preparation-for-upgrade.html[complete the preparation for upgrade].

// 2026 JUN 2, AFFFASDOC-477
