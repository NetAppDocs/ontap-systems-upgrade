...
Check if the build on node3 and node4 contains a NODAR (no data at rest) image. If the build contains a NODAR image, you need to reinstall an image containing DARE (data at rest encryption).
...
systemshell
----
. [[check-build]]Verify the build by running the following command and examining the output:
+
[source,cli]
----
# cat /BUILD
----
+
If `build-type` in the output contains `NODAR`, the build contains a NODAR image. If `build-type` does not contain `NODAR`, the build contains a DARE image.
+
. If the build contains a NODAR image, reinstall an image that contains DARE.
...
halt
---- 
. Reinstall the image containing DARE:
+
<add_command>
