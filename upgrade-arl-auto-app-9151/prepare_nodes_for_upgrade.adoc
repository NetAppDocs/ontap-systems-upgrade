...
Check if the build on node3 and node4 contains a NODAR (no data at rest) image. If the build contains a NODAR image, reinstall an image on node3 and node4 that contains DARE (data at rest encryption).

.Steps

. Boot node3 and node4 to the boot menu:
+
[source,cli]
----
...
systemshell
----
. [[check-build]]Check the build on node3 and node4:
+
[source,cli]
----
# cat /BUILD
----
+
If `build-type` in the output contains `NODAR`, the build contains a NODAR image (no encryption). If `build-type` does not contain `NODAR`, the build contains a DARE image (encryption).
+
. If the build contains a NODAR image, reinstall an image on the node that contains DARE.
