...
summary: "Install and boot node3 and install ONTAP software when upgrading to controllers introduced in ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
. When node3 stops at the boot menu, reassign the node1 disks to node3 by running the following command on node3:
+
...
. Enter `Ctrl-C` during autoboot to stop the node at the `LOADER>` prompt.
...
.What's next?
After installing and booting node3, link:verify_node3_installation.html[verify the node3 installation].
