...
summary: "Verify that the node1 NAS data LIFs on node2 are relocated from node2 to node3 when upgrading to controllers introduced in ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
. If necessary, restore and revert displaced LIFs or manually migrate and modify the node1 LIFs that failed to relocate automatically to node3.
+
[role="tabbed-block"]
====
...
--
.. Migrate the LIFs that failed to relocate automatically to node3:
+
[source,cli]
----
...
.What's next?
After moving non-root aggregates and NAS data LIFs owned by node1 from node2 to node3, link:relocate_non_root_aggr_nas_lifs_from_node2_to_node3.html[relocate non-root aggregates and NAS data LIFs from node2 to node3].
