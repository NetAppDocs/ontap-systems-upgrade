...
summary: "Enable the HA pair, verify node3 and node4 storage access, and confirm aggregate ownership after upgrading to controllers introduced in ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
+
Refer to link:other_references.html[References] to link to the _System Administration Reference_ for information about the SPs and the _ONTAP 9 Command reference_ for detailed information about the system `service-processor network modify` command.
include::../_include/ru_auto_ensure_new_controllers_are_set_up_correctly_step_14_end.adoc[]

.What's next?
After confirming that the new controllers are set up correctly, link:set_up_storage_encryption_new_module.html[set up Storage Encryption on the new controller module].
