...
summary: "If required, restore key-manager configuration on node4 when upgrading to controllers introduced in ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
.What's next?
After restoring key-manager configuration on node4, link:move_non_root_aggr_and_nas_data_lifs_node2_from_node3_to_node4.html[move non-root aggregates and NAS data LIFs owned by node2 from node3 to node4].
