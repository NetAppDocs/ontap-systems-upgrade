...
. [[step2]]In maintenance mode (option 5 at boot menu), modify the FC ports on node3 as needed:
+
...
. [[step4]]After you enter the command, wait until the system stops at the boot environment prompt.
...
[start=12]
. [[auto_check3_step12]]Go to link:verify_node3_installation.html[Verify the node3 installation].
...
. If the system goes into a reboot loop with the message `no disks found`, it indicates that there was a problem with the disk reassignment, which you must link:troubleshoot_index.html[troubleshoot].
