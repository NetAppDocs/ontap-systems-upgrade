...
. If the ports are not set to storage, those can be modified to storage using command 'storage port modify -p <port> -m storage'
+
All ethernet ports connected to storage shelves must be configured as storage in order to be able to access disks/shelves. 
...
+
If the root volume is encrypted, you must recover the onboard key-management secrets so the system can find the root volume. 
+ 
