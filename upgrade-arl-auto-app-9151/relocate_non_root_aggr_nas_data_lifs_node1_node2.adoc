...
summary: "Move non-root aggregates and NAS data LIFs from node1 to node2 and move node1's resources to node3 when upgrading to controllers introduced in ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
.What's next?
After relocating non-root aggregates and NAS data LIFs owned by node1 to node2, link:relocate-failed-or-vetoed-aggr-node2.html[relocate failed or vetoed aggregates to node2].
