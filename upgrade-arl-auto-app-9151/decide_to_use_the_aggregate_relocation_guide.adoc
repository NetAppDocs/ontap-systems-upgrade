...
summary: "Decide whether to use ARL to upgrade to controllers introduced ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
[.lead]
There are several aggregate relocation (ARL) methods for upgrading controller hardware. This ARL procedure describes how to upgrade an HA pair of controllers in AFF and FAS storage systems running ONTAP 9.15.1 or later to new controllers introduced in ONTAP 9.15.1 or later, while keeping the existing disks and the data stored on the disks. This is a complex procedure that should be used only by experienced ONTAP administrators. 
...
|===
+
To perform an upgrade listed in the above table, see link:other_references.html[References] to link to _Use "system controller replace" commands to upgrade controller models in the same chassis_.
