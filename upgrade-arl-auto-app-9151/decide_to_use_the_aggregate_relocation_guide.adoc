...
summary: "Decide whether to use ARL to upgrade to controllers introduced in ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
[.lead]
There are several aggregate relocation (ARL) methods for upgrading controller hardware. This ARL procedure describes how to upgrade an HA pair of controllers in AFF and FAS storage systems to controllers introduced in ONTAP 9.15.1 or later, while keeping the existing disks and the data stored on the disks. This is a complex procedure that should be used only by experienced ONTAP administrators. 
...
|===
+
To perform an upgrade listed in the above table, see link:other-references.html[References] to link to _Use "system controller replace" commands to upgrade controller models in the same chassis_.
* You're upgrading a MetroCluster FC or IP configuration. To upgrade a MetroCluster configuration, see link:other-references.html[References] to link to the _MetroCluster Upgrade and Expansion documentation_.
...
.What's next?
After deciding to use this ARL procedure, link:required_tools_and_documentation.html[learn about the required tools and documentation].

// 2025 OCT 22, AFFFASDOC-405
// 2025 APR 15, ontap-systems-upgrade/issue-125
