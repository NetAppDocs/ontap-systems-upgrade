...
summary: "Decide whether to use ARL to upgrade to controllers introducced ONTAP 9.15.1 or later by using `system controller replace` commands."
---
...
[.lead]
There are several aggregate relocation (ARL) methods for upgrading controller hardware. This ARL procedure describes how to upgrade to an HA pair of controllers in AFF and FAS storage systems introduced in ONTAP 9.15.1 or later with new controllers while keeping the existing disks and the data stored on the disks. This is a complex procedure that should be used only by experienced ONTAP administrators. 
