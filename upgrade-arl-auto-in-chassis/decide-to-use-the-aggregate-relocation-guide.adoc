...
[.lead]
There are several aggregate relocation (ARL) methods for upgrading controller hardware. This content describes how to upgrade storage controllers in an HA pair by converting the existing system to the replacement system, keeping the existing system chassis, disks, and stored data. You should only use this complex procedure if you're an experienced ONTAP administrator.
