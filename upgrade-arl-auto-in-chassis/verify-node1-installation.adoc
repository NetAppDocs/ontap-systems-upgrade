...
Verify that node1 is in quorum. Then, resume the controller replacement operation.
...
.	With the controller replacement in a paused state, proceed to <<restore_network_configuration_on_node1,Restore network configuration on node1>>.
...
.About this task
For more information on creating and re-creating VLANs, interface groups, and broadcast domains, refer to link:other-references.html[References] to link to the _Network Management_ content.
...
.What's next?
After verifying the node1 installation, link:restore-key-manager-config-upgraded-node1.html[restore key-manager configuration on the upgraded node1].
