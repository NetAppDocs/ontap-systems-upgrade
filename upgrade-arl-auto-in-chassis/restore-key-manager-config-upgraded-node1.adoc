...
.What's next?
After restoring key-manager configuration on the upgraded node1, link:move-non-root-aggr-nas-lifs-node1-from-node2-to-upgraded-node1.html[move node1 non-root aggregates and NAS data LIFs from node2 to the upgraded node1].
