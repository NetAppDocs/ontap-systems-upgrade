...
. If necessary, restore and revert displaced LIFs or manually migrate and modify the node1 LIFs that failed to relocate automatically to node1.
+
[role="tabbed-block"]
====
...
--
.. Migrate the LIFs that failed to relocate automatically to node1:
+
[source,cli]
----
...
.What's next?
After moving node1 non-root aggregates and NAS data LIFs from node2 to the upgraded node1, link:relocate-non-root-aggr-nas-lifs-from-node2-to-node1.html[relocate non-root aggregates and NAS data LIFs from node2 to node1].
