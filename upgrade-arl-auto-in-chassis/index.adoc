...
[.lead]
There are several aggregate relocation (ARL) methods for upgrading controller hardware. This procedure describes how to upgrade storage controllers in an HA pair by converting the existing system to the replacement system, keeping the existing system chassis, disks, and the data stored on the disks.
...
NOTE: The terms *node1* and *node2* are used only as a reference to node names in this document. When following the procedure, you must use the actual names of your nodes.

== Requirements and limitations

You need to consider important factors before starting the upgrade. 

NOTE: You must review all the following important information.

* This procedure is complex and assumes that you have advanced ONTAP administration skills. You should read and understand the link:guidelines-for-upgrading-controllers-with-arl.html[guidelines for upgrading controllers] and link:overview-of-the-arl-upgrade.html[ARL upgrade sequence] before starting the upgrade.
...
* If you have a switch that is not supported by the ONTAP version and the replacement system that you are upgrading to, refer to link:other-references.html[References] to link to the _Hardware Universe_.
...
* If you are upgrading from an existing system with downtime that is included in the link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis[supported systems matrix], you can upgrade controller hardware by moving storage or by contacting technical support. Refer to link:other-references.html[References] to link to _Upgrade by moving volumes or storage_.
...
.What's next?
After learning about this ARL upgrade procedure, link:decide-to-use-the-aggregate-relocation-guide.html[decide whether to use this aggregate relocation procedure].
