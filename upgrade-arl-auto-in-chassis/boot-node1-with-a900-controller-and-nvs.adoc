---
sidebar: sidebar
permalink: upgrade-arl-in-chassis/boot-node1-with-a900-controller-and-nvs.html
...
[.lead]
Node1 with the replacement modules is now ready to boot. The supported replacement modules are listed in the link:index.html#supported-systems,[supported systems matrix].
