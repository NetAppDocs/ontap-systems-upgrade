...
Verify that node2 is in quorum. Then, resume the controller replacement operation.
...
. With the controller replacement in a paused state, proceed to <<restore_network_configuration_on_node2,Restore network configuration on node2>>.
...
.What's next?
After verifying the node2 installation, link:restore-key-manager-config-node2.html[restore key-manager configuration on node2].
