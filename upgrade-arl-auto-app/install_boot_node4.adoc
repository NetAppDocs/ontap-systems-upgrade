...
---
= Install and boot node4 (ONTAP 9.8 or later, automated)
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
+
NOTE:  You must use the management port and IP as the netboot connection. Don't use a data LIF IP or a data outage might occur while the upgrade is being performed.
...
[start=22]
. [[auto_install4_step22]]If you have NetApp Storage Encryption (NSE) drives installed, perform the following steps:
+
...
+
Enter the passphrase and the backup information that you recorded earlier in the procedure. Refer to link:complete-preparation-for-upgrade.html#manage_storage_encryption_using_okm[Manage storage encryption using the Onboard Key Manager].
...
* If you have an FC or UTA/UTA2 configuration, link:set_fc_or_uta_uta2_config_node4.html[set and configure the FC or UTA/UTA2 ports on node4].

* If you don't have an FC or UTA/UTA2 configuration, link:reassign-node2-disks-to-node4.html#reassign-node2-node4-app-step1[reassign node2 disks to node4, Step 1] so that node4 can recognize node2's disks.

* If you have a  MetroCluster configuration, link:set_fc_or_uta_uta2_config_node4.html[set and configure the FC or UTA/UTA2 ports on node4] to detect the disks attached to the node.
