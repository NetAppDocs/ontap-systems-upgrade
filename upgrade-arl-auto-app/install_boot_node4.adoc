...
`boot_ontap menu`
+
If you don't have an FC or UTA/UTA2 configuration, preform link:reassign-node2-disks-to-node4.html#reassign-node2-node4-app-step1[Reassign node2 disks to node4, Step 1] so that node4 can recognize node2's disks.

.  [[step29]]For MetroCluster configuration, you must set and configure the FC or UTA/UTA2 ports on node4 to detect the disks attached to the node.
