...
= Relocate non-root aggregates and NAS data LIFs from node2 to node3 (ONTAP 9.8 or later, automated)
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
.What's next?
After relocating non-root aggregates and NAS data LIFs from node2 to node3, link:relocate-failed-or-vetoed-aggr-node3.html[relocate failed or vetoed aggregates].
