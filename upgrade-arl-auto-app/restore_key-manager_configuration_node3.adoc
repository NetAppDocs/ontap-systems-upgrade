...
= Restore key-manager configuration on node3 (ONTAP 9.8 or later, automated)
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
.What's next?
After restoring the key-manager configuration on node3, link:move_non-root_aggr_and_nas_data_lifs_node1_from_node2_to_node3.html[move non-root aggregates and NAS data LIFs owned by node1 from node2 to node3].
