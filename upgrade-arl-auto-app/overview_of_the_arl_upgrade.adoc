...
---
= Learn about the ARL upgrade sequence (ONTAP 9.8 or later, automated)
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
| link:prepare_nodes_for_upgrade.html[Stage 1. Prepare for upgrade]
...
| link:relocate_non_root_aggr_nas_data_lifs_node1_node2.html[Stage 2. Relocate and retire node1]
...
| link:relocate_non_root_aggr_nas_lifs_from_node2_to_node3.html[Stage 4. Relocate and retire node2]
...
a| During Stage 5, you install and boot node4, check that the cluster and node-management ports from node2 come online on node4, reassign the node2 disks to node4, and verify the node4 installation. If you are using NVE, you restore key-manager configuration. If required, you set the FC or UTA/UTA2 configuration on node4. You also relocate node2 NAS data LIFs and non-root aggregates from node3 to node4 and verify that the SAN LIFs exist on node4.
...
| link:manage-authentication-using-kmip-servers.html[Stage 6. Complete the upgrade]
