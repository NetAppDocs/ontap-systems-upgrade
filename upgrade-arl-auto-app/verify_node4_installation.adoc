...
If the physical ports on node2 do not map directly to the physical ports on node4, the subsequent section <<restore_network_configuration_on_node4,Restore network configuration on node4>> must be used to repair network connectivity.
...
.What's next?
After verifying the node4 installation, link:restore_key-manager_config_node4.html[restore the key-manager configuration on node4].
