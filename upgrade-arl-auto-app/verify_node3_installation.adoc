...
If the physical ports on node1 do not map directly to the physical ports on node3, the subsequent section <<restore_network_configuration_on_node3,Restore network configuration on node3>> must be used to repair the network connectivity.
...
`network port reachability show`
+
As the reachability repair is performed, ONTAP attempts to place the ports in the correct broadcast domains. However, if ONTAP cannot determine a port’s reachability and it does not belong to any of the existing broadcast domains, ONTAP creates new broadcast domains for these ports.
...
.What's next?
After verifying the node3 installation, link:restore_key-manager_configuration_node3.html[restore the key-manager configuration on node3].
