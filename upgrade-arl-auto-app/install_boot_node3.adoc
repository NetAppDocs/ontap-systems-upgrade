...
`boot_ontap menu`
+
If you don't have an FC or UTA/UTA2 configuration, perform link:reassign-node1-disks-to-node3.html#reassign-node1-node3-app-step1[Reassign node1 disks to node3, Step 1] so that node3 can recognize node1's disks.

. [[step29]]For a MetroCluster configuration,  you must set and configure the FC or UTA/UTA2 ports on node3 to detect the disks attached to the node.
