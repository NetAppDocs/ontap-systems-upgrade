...
. If the system goes into a reboot loop with the message `no disks found`, it indicates that the system has reset the FC or UTA/UTA2 ports back to the target mode and therefore is unable to see any disks. Perform one of the following to resolve this issue:
* Continue with <<reassign-node1-node3-app-step3,Step 3>> to <<reassign-node1-node3-app-step8,Step 8>>
