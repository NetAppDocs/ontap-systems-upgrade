...
[.lead]
There are several aggregate relocation (ARL) methods for upgrading controller hardware. This ARL procedure describes how to upgrade the storage controllers in an HA pair with new controllers while keeping the existing data and disks. You should only use this complex procedure if you are an experienced administrator.
