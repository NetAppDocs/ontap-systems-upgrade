...
= Relocate non-root aggregates and NAS data LIFs owned by node1 to node2 (ONTAP 9.8 or later, automated)
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
.What's next?
After relocating non-root aggregates and NAS data LIFs owned by node1 to node2, link:relocate-failed-or-vetoed-aggr-node2.html[relocate failed or vetoed aggregates].
